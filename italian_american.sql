CREATE OR REPLACE TABLE `stac-labs.ga_sharing.italian_american_targets` AS
SELECT
    ts.*,
    p.sos_id,
    p.myv_van_id,
...
WHERE ts.sub_ethnicity IN ('Italian')
AND p.person_id IS NOT NULL
AND p.is_deceased = false
AND p.ethnicity_modeled_confidence_level IN ('Medium','High')
